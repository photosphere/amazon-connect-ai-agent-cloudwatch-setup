...
(function () {
  ...
  function computeMetrics(list) {
    ...
    // SR 信息提取回答轮次: 若触发过 requestRepair，统计到最后一次 requestRepair 之前
    // 客户提供信息的轮次(客户 UTTERANCE 数量)。
    let srRounds = null;
    if (srIntent > 0) {
      let rounds = 0;
      for (const e of list) {
        if (e.type === "UTTERANCE" && /^\[CUSTOMER\]/.test((e.obj && e.obj.utterance) || "")) {
          if (lastRepairTs == null || e.ts <= lastRepairTs) rounds++;
        }
      }
      srRounds = rounds;
    }
  ...
  function renderDashboard() {
    ...
    for (const row of rows) {
      ...
      // 比率格式化
      const srRate = mm.srIntent ? Math.round((mm.srSuccess / mm.srIntent) * 100) + "%" : t("na");
